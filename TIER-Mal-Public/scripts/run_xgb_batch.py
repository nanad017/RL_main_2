...
XGB_DIR = Path(os.getenv(
    "XGB_DETECTOR_DIR",
    TIER_ROOT / "detectors" / "custom_rl" / "xgboost",
)).expanduser().resolve()
...
def main():
    ...
    model_path = Path(os.getenv(
        "CUSTOM_DETECTOR_MODEL_PATH",
        XGB_DIR / "artifacts/binary_xgboost.joblib",
    )).expanduser().resolve()
    metrics_path = Path(os.getenv(
        "XGB_METRICS_PATH",
        XGB_DIR / "artifacts/metrics_xgboost.json",
    )).expanduser().resolve()
