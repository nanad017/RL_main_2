...
def create_vectorized_features(data_dir: Path | str, label_type: str = "label", class_min: int = 10) -> None:
    ...
    if label_type != "label": # No work needed for the default malicious/benign labels
        ...
        for l in sorted(k for k in train_label_counts if k is not None):
            count = train_label_counts[l]
            if l in ignore_tags:
                continue
            if count >= class_min:
                label_map[l] = i
    ...
    if label_type != "label": # No work needed for the default malicious/benign labels
        ...
        # Remove labels/tags that appear fewer than class_min time
        for l in sorted(k for k in test_label_counts if k is not None):
            count = test_label_counts[l]
            if l in ignore_tags:
                continue
            if label_map.get(l) is not None:
                continue
        ...
        with (data_path / f"{label_type}_labels.json").open("w") as fout:
            json.dump({"labels": ordered_labels}, fout, indent=2)
    ...
    if nrows == 0:
        print("Challenge set rong, bo qua vectorize challenge")
    else:
