...
def test_xor_sub_scope_is_full_state():
    ...
    # dist_id is recorded and reproducible-by-seed.  The format embeds the
    # emulation WIDTH (unicorn32/unicorn64), the full-state boundary+uniform
    # battery, a memory-write marker (+mem), the draw count, and the per-call
    # urandom seed: e.g. "unicorn64-fullstate-boundary+uniform+mem-n128-seed0x...".
    # Assert the load-bearing, WIDTH-AGNOSTIC pieces rather than a brittle exact
    # prefix: full-state scope ("fullstate-boundary"), the uniform draws
    # ("+uniform"), the memory channel ("+mem"), and reproducible-by-seed
    # ("seed0x").  This still pins "full-state scope, reproducible by seed".
    did = ev.scope.input_dist
    assert "fullstate-boundary" in did, did
    assert "+uniform" in did, did
    assert "+mem" in did, did
    assert "seed0x" in did, did
