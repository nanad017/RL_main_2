...
def test_xor_sub_pair_yields_z3_unsat_proof(oracle):
    ...
    z3 is harder to skip than a plain ``importorskip``: in a non-conda env the
    ``z3-solver`` wheel IMPORTS fine but its bundled ``libz3.so`` fails to load
    at first CALL (Z3Exception / OSError), which previously surfaced as a test
    FAILURE and misled a reviewer.  We first prove z3 can actually be USED (a
    trivial solver call exercises the native lib); if that raises we ``skip``
    cleanly — it is an ENVIRONMENT problem, not a funcval defect.  The real
    assertion path below is unchanged when z3 works.
    """
    z3 = pytest.importorskip("z3")
    # Force the native libz3.so to load via a real solver call. If z3 imported
    # but its shared lib is broken, this raises here -> clean skip, not a fail.
    try:
        _s = z3.Solver()
        _s.add(z3.BitVec("p", 8) == 0)
        assert _s.check() == z3.sat
    except (z3.z3types.Z3Exception, OSError, Exception) as exc:  # noqa: BLE001
        pytest.skip(f"z3 present but native lib failed to load: "
