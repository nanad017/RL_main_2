...
UTIL_PATH = os.path.join(PROJECT_ROOT, "models")
...
def generate_example_benign_strings_output(benign_repo=None, output_dir=None):
    ...
    file = open(
        os.path.join(PROJECT_ROOT, "actions", "ls", "good_strings", "xournal-strings.txt"),
        "w",
    )
    ...
    shutil.move(
        ...
        os.path.join(
            MODULE_PATH,
            os.path.join(PROJECT_ROOT, "actions", "ls", "trusted", unzipped_filename),
        ),
    )
    ...
    if args.accept:
        if check_if_samples_exist(directory_to_check=SAMPLE_PATH) | args.force is True:
            ...
            print(
                ...
                "directory (data/samples) already, aborting"
