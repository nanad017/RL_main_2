...
class TestDefault:
    ...
    def test_evasion_default(self):
        ...
        assert reward == pytest.approx(9.9 + 1 / 3)
        assert done is True
        assert isinstance(reward, float)
        assert isinstance(done, bool)
...
class TestRFunc:
    def test_stoke_funcval_pass_has_no_integrity_penalty(self):
        reward, done = make_call(
            TierAwareReward(),
            binary=VALID_PE,
            action_name="stoke_rewrite",
            action_context={
                "changed": True,
                "funcval": {"ran": True, "passed": True},
            },
        )
    ...
    def test_stoke_missing_funcval_verdict_is_penalized(self):
        reward, done = make_call(
            TierAwareReward(),
            binary=VALID_PE,
            action_name="stoke_rewrite",
            action_context={"changed": True},
        )

        assert reward == pytest.approx(0.2 - 15.0)
        assert done is False

    def test_checker_receives_action_context(self):
        seen = {}

        def checker(binary, action_name, action_context):
            seen.update(action_name=action_name, action_context=action_context)
            return True

        context = {"funcval": {"passed": True}}
        make_call(
            TierAwareReward(check_func=checker),
            action_name="stoke_rewrite",
            action_context=context,
        )

        assert seen == {"action_name": "stoke_rewrite", "action_context": context}
